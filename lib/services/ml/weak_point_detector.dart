...
class WeakPointDetector {
  ...
  Map<String, dynamic> _analyzeByCategoryAndLevel(
    ...
    for (final review in reviews) {
      ...
      if (review.quality < 3) {
        ...
        if (!(stats[key]!['strugglingIds'] as List<int>).contains(review.vocabularyId)) {
    ...
    for (final entry in stats.entries) {
      ...
      data['avgResponseTime'] = data['totalResponseTime']! / data['totalAttempts']!;
    }
  ...
  double _calculateSeverity(Map<String, dynamic> stats, double errorRate) {
    ...
    final normalizedResponseTime = 
        min((avgResponseTime - expectedResponseTime) / expectedResponseTime, 1.0)
            .clamp(0.0, 1.0);
    
    final severity = (normalizedErrorRate * errorWeight) +
  ...
  List<int> getPriorityReviewItems(
  ...
  Map<String, dynamic> generateInsights(List<WeakPoint> weakPoints) {
    if (weakPoints.isEmpty) {
      return {
        ...
        'recommendations': ['Keep up the great work! Continue regular reviews.'],
      };
    }
    
    final severeWeaknesses = weakPoints.where((wp) => 
        wp.errorRate >= severeWeaknessThreshold).toList();
    ...
    if (weakPoints.length > 3) {
      recommendations.add('Consider reducing daily review volume and focus on mastery');
    }
    
    return {
      'overallStatus': severeWeaknesses.isNotEmpty ? 'needs_attention' : 'improving',
